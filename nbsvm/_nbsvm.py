...
class NBSVM(BaseEstimator, LinearClassifierMixin):
	"""
	A NBSVM classifier following the sklearn API, as described in Section 2.3 of
	...
	def _log_count_ratio(self, X, y):
		"""
		Log-count ratio computed from smoothed (by alpha) count vectors for each
		class. These are the coefficients in pure Multinomial Naive Bayes.
		"""
	...
	def _interpolate(self, coef):
		"""
		Interpolate with parameter beta between Multinomial Naive Bayes		(mean_weight) and SVM.
		"""
